...
| CFG-004 | MAYOR | ✅ REAL | Líneas 113-114, 143-144: `let` para variables globales | `_SHEETS_CACHE` y `_SPREADSHEET_CACHE` son globales mutables. En Apps Script pueden causar condiciones de carrera si múltiples usuarios ejecutan código simultáneamente. |
...
| CFG-009 | MAYOR | ✅ REAL | Líneas 173-195: getSheet() cachea sin invalidación | El cache `_SHEETS_CACHE` nunca se invalida. Si la hoja cambia externamente, el cache queda desactualizado. |
| CFG-010 | MAYOR | ✅ REAL | Líneas 188-191: FIFO eviction elimina primera clave arbitrariamente | Cuando hay 20 hojas en cache, elimina la primera sin considerar uso. Puede causar inconsistencia si esa hoja sigue en uso. |
