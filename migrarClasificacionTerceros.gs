/**
 * Script de migración V1: Clasificación de Terceros + Producto_Proveedor.
 * Idempotente — verifica flag MIGRACION_TERCEROS_V1_DONE en ScriptProperties.
 * Crea la hoja Producto_Proveedor y backfillea tipoTercero en registros existentes.
 */
function migrarClasificacionTerceros() {
  const MIGRATION_FLAG = 'MIGRACION_TERCEROS_V1_DONE';
  ...
  Logger.log('[MIGRACION] Flag MIGRACION_TERCEROS_V1_DONE = true');
 ...
 */
function revertirMigracionTercerosV1() {
  const MIGRATION_FLAG = 'MIGRACION_TERCEROS_V1_DONE';
  const props = PropertiesService.getScriptProperties();
  if (props.getProperty(MIGRATION_FLAG) !== 'true') {
    return { success: false, message: 'No hay migración V1 para revertir.' };
  }

  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const snapshotSheet = ss.getSheetByName('SNAPSHOT_TERCEROS_V1');
  if (!snapshotSheet || snapshotSheet.getLastRow() < 2) {
    return { success: false, message: 'No se encontró snapshot SNAPSHOT_TERCEROS_V1.' };
  }
  ...
  Logger.log('[MIGRACION] Reversión completada — datos de Terceros restaurados desde snapshot.');
